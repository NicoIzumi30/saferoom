...
const rangeInput2 = document.querySelectorAll("#range-input input"),
    priceInput2 = document.querySelectorAll("#price-input input"),
    range2 = document.querySelector("#slider .progress");
let priceGap2 = 1000;
priceInput.forEach(input => {
    input.addEventListener("input", e => {
        ...
        if ((maxPrice - minPrice >= priceGap2) && maxPrice <= rangeInput2[1].max) {
            if (e.target.className === "input-min") {
                ...
                range.style.left = ((minPrice / rangeInput2[0].max) * 100) + "%";
            } else {
                rangeInput2[1].value = maxPrice;
                range.style.right = 100 - (maxPrice / rangeInput2[1].max) * 100 + "%";
            }
        }
    });
});
